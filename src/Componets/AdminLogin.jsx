...
const handelSubmit = (e) => {
    ...
    adminLogin(email, password)
        .then((result) => {
            console.log(result);
            if(result.token){
            Cookies.set('adminId', result.token, { expires: 7 });
            enqueueSnackbar('Authentication Completed..!',{variant:"success"});
                window.location.reload();
                window.location.href='/'
            }else{
                enqueueSnackbar('Authentication Failed',{variant:"success"});
            }
        }
    )

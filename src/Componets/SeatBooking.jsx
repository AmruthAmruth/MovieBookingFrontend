...
import { createBooking, getBookingByMovieName, verifyPayment } from '../Api-Helpers/Api-helpers';
...
const navigate=useNavigate()
    ...
    const handleSubmit = (seats, e) => {
        e.preventDefault();
        
        const amount = selectedSeat.length * 150;
    ...
    { userId ?   createBooking(userId, seatNumber, amount, movieName, bookingOptions)
            .then((result) => {
                console.log(result.data.order);
                const paymentOptions = {
                    "key": "rzp_test_KmTrLxqsQmvXcM", // Enter the Key ID generated from the Dashboard
      ...
      : enqueueSnackbar('Please  Login Your Account',{variant:"success"})}   
    };
...
const soldSeat=(val)=>{
    if(bookedSeats.includes(val)){
        enqueueSnackbar('Already Booked This Seat',{variant:"success"});
    }else{
      addSeat(val)
    }
    ...
    return (
        ...
        <div class="container mx-auto px-4  dark:bg-gray-900">
            ...
            <div class=' text-center'>
                <form action="">
                    <button type="button" class="text-gray-900  mt-2 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 w-[15rem] py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">Seat Number : {selectedSeat ? selectedSeat + "  " : ""}</button> <br />
                    <button type="button" class="text-gray-900 mt-2 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 w-[15rem] py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">Total Price : {selectedSeat ? selectedSeat.length * 150 : ""}</button> <br />
                    <button onClick={(e) =>handleSubmit(selectedSeat, e)}
                        type="submit" class="w-[20rem] mt-4 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Pay Now</button>
